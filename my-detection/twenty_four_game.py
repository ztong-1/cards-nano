...
#s = Solution()
#res = s.judgePoint24([2,3,4,1])
#if res == False:
#    print("No solution exists.")
#print("\n\n")

class Solution2:
    def judgePoint24(self, cards:[int]) -> str:
        def process_ops(ops):
            res = "(" + ops[-1][1] + ")"
            for i in range(len(ops)-2, -1, -1):
                if "+" in ops[i][1] or "-" in ops[i][1]:
        ...
        def dfs(x, y, remain, ops):
            ...
            if len(remain) == 2:
                ...
                temp = remain[0] + remain[1]
                if temp == x_y:
                    res = process_ops(ops + [("", str(remain[0])+"+"+str(remain[1]))])
                    #print(res)
                    return res
                temp = remain[0] - remain[1]
                if temp == x_y:
                ...
                temp = remain[1] - remain[0]
                if temp == x_y:
                    res = process_ops(ops + [("", str(remain[1])+"-"+str(remain[0]))])
                    #print(res)
                    return res
                temp = remain[1] * remain[0]
                if temp == x_y:
